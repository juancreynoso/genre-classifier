...
\draw[->] (csv1) -- (model);
\draw[->] (csv2) -- (model);
\draw[->] (model) -- node[above,font=\tiny]{2, 4b, 5, 7} (exp);
\end{tikzpicture}
\end{center}
...
\begin{itemize}[leftmargin=0.7cm]
  ...
  scripts de folklore (3, 4, 4b y 7) a través de \texttt{features.py}, que extrae
...
(scripts 2, 5, 7 y la parte de modelado de 4b) ya \textbf{no} tocan el audio.
...
\begin{defenbox}{La red neuronal y la CNN no se guardan en disco}
Los únicos modelos persistidos son los \textbf{dos SVM}. La \textbf{red neuronal}
se entrena en memoria dentro de los scripts 5 y 7 (se usa y se descarta en cada
corrida), y la \textbf{CNN} fue descartada y removida del repositorio (queda solo
\texttt{6\_cnn\_training.py} como referencia). Esto es a propósito: el foco del
proyecto es el \emph{análisis}, no servir predicciones en producción.
\end{defenbox}

\subsection{¿Hace falta el audio una vez que tengo el CSV?}

\textbf{Para reproducir los experimentos y entrenar: no.} Los scripts 2, 4, 4b, 5
y 7 \textbf{solo leen los CSVs}, nunca abren un \texttt{.wav}. Con los dos CSVs
...
El script \texttt{4b\_folklore\_crossvalidation.py} arregla esto. La idea de la
...
un Perceptrón Multicapa (MLP) con Keras sobre las \emph{mismas} 45 features.
...
model = keras.Sequential([
    ...
    layers.Dense(10, activation='softmax')   # 10 generos
])
\end{lstlisting}
...
\begin{description}[leftmargin=0.5cm,style=nextline]
  ...
  \item[Capa de salida (10, softmax)] \textbf{Softmax} convierte las 10 salidas
...
\begin{description}[leftmargin=0.4cm,style=nextline]
  ...
  Por eso hicimos la versión correcta (\texttt{4b}) con \textbf{cross-validation
